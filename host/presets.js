(() => {
    ...
    function presetView(name, preset) {
        ...
        const entries = preset.prompts.filter(entry => entry && typeof entry.identifier === 'string')
            .map(entry => ({ identifier: entry.identifier,
        ...
        // 优先采用酒馆的默认角色顺序，未列入顺序的条目仍可读取。
        const orders = Array.isArray(preset.prompt_order) ? preset.prompt_order : [];
        const order = orders.find(item => String(item?.character_id) === '100001')?.order;
    ...
    function createPresets(getContext) {
        ...
        return {
            ...
            async readPreset(name) {
                ...
                return presetView(name, getSaved(manager, name));
            },
            ...
            async savePresetEntry({ presetName, identifier, content, expectedContent } = {}) {
                ...
                const result = presetView(presetName, next);
                saving = true;
                try {
