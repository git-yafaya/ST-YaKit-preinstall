(() => {
    ...
    <div id="presets-page" class="page" tabindex="-1" hidden>
      <section class="panel" aria-label="预设展示">
        <div class="panel-content preset-picker">
          <div class="field"><label for="preset-name">Chat Completion 预设 <span id="preset-entry-count" class="subtle-badge">0 个条目</span></label>
            <div class="save-row"><select id="preset-name"></select><button type="button" id="refresh-presets" class="button button-secondary">刷新列表</button><button type="button" id="read-preset" class="button button-secondary">重新读取</button></div>
          </div>
  ...
  workbench.presetSaveTemplate = `
    <div class="preset-save">
      ...
      <button type="button" id="save-preset-entry" class="button button-secondary">保存到原条目</button>
    </div>
  `;
})();
