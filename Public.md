...
| 发布与验收 | v0.1.0 交互预览，尚未正式发布；直接打开文件与界面交互待用户人工确认 |
...
核心与本地示例测试共 9 项通过，全部 JavaScript 语法检查与 `git diff --check` 通过。语法和本地测试结果不代表界面验收完成。

界面验收由用户人工完成，直接打开 `index.html` 与实际交互尚待确认。按 README「使用说明」完成一轮，预期保留两版提示词、各自的试写和人工反馈；同时确认取消、复制、导出及重新打开恢复。SillyTavern 相关验收遵循本机 `AGENTS.md` 指定的人工验收流程。
