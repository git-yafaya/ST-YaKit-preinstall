...
设置一级页提供默认收起的「界面设置」「副 API」「提示词」分组。API 选择框下列出已存配置卡片；新增后立即选中并使用副 API，编辑保留 ID、列表位置和当前主副模式，编辑非激活项不改变当前连接。删除激活项后选择剩余首项，删除最后一项回主 API。选择和编辑同步 `secondary*` 旧字段，旧 `update` 调用修改这些字段时也同步当前列表项。
