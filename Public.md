...
│   ├── settings-view.js            # 导航设备适配、设置子页与顶栏保存操作
...
│   ├── settings-ui.test.mjs        # 设置草稿、过期请求与自动导航检查
...
按钮沿用主页的圆角、间距、150ms 颜色过渡与按下缩放，禁用态继续由原有状态控制。设置页的「配置 API」按钮位于「使用配置」选择框右侧，两者同高对齐。预设条目名称可换行，操作按钮行空间不足时自动换行。非提交按钮显式使用 `type="button"`；设计表单由提交按钮触发，设置编辑由顶栏按钮确认。折叠入口保留原生 `details/summary`，反馈和导航位置保留原生单选输入，键盘操作由浏览器处理。
...
| `tests/settings-ui.test.mjs` | PC、手机、平板、iPadOS 自动导航与手动覆盖；连接及模型结果过期、密钥解除引用、输入保留与提示词草稿 |
...
v0.3.3 已通过导航默认值与持久化、11 类设备的自动位置及手动覆盖检查，以及既有导航键盘、焦点、输入保留和样式规则检查。人工验收时，在 PC、手机和平板选择「自动」，确认分别使用上方和下方导航；再切换手动位置并重新加载，核对选择保留、五页尺寸和切页动画。
