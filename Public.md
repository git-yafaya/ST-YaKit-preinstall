...
常驻导航依次提供预设预览、工作台、试写与反馈、版本记录、设置五个页面，页面轨道采用相同顺序；上方显示文字，下方显示图标并保留可访问名称。工作台在宽屏将左侧需求输入与生成操作、右侧修改讨论等宽等高铺满可用区域，小屏上下排列；五页共用随宿主视口调整大小的窗口与微动效。试写页使用冷浅色双栏布局，其余页面沿用所选主题，样式保存在本仓库。
...
| 试写双栏 | 视口宽度 ≥960px 时，`.trial-panel` 左侧约 42% 为参数与创建区，右侧约 58% 为任务与阅读区，分隔线为 `1px solid rgba(0,0,0,.06)`；≤959px 上下等分；窄屏且高度 ≤600px 时任务标签与选择框横排，收紧常驻栏留白 |
...
| 试写色彩与文字 | 底色 `#F8F9FA`，表单和卡片为纯白，边框 `rgba(0,0,0,.08)`；创建按钮至少 40px 高，`#18181B` 配白字；现代无衬线字体和 `-.01em` 字距，提示 12px / `#6B7280`，占位文字 `#9CA3AF`；Aa 空态保留衬线字体 |
| 试写导航 | 冷灰底槽、白色选中卡片，选中项 500 字重与 `0 1px 3px rgba(0,0,0,.08)` 投影；导航位置和位移动效沿用主界面 |
...
| 文本输入框 | 通用样式聚焦时改变边框颜色；试写页输入框和下拉框聚焦增加柔和 2px 外圈，文本框限制高度并关闭拖动缩放；按钮、复选框和单选项保留键盘焦点提示 |
| 数字输入框 | `.yakit-workbench` 内所有 `input[type="number"]` 隐藏浏览器原生上下步进按钮，直接输入数值；保留数字类型及 `min`、`max`、`step` 校验 |
| 选择卡片 | 空卡模式、合并生成和评分排序共用 `.test-toggle` 标签卡片，整块点击切换原生复选框；通用样式 12px 圆角，试写页使用 8px 圆角和中性冷灰配色；点击时保持尺寸稳定，文字、背景和边框颜色使用 150ms 过渡；选中、键盘聚焦和禁用状态由内部输入控件决定，长标题在窄屏换行 |
...
`style.css` 最后加载 `styles/trial.css`。试写页直接覆盖自身色彩变量；导航的试写项为 `aria-selected="true"` 时，CSS 同时为弹窗底色、顶栏和分段导航应用冷浅色外观。切页后其他页面使用保存的主题选择，`data-theme` 与持久化设置沿用原流程。原任务、样本、评分与反馈控件的 ID、选项值、事件和禁用条件全部保留；没有任务时收起任务详情区，Aa 空态填充阅读区，有结果时正文和反馈在同一阅读区展示。
