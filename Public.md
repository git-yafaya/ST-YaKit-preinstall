...
| 选择卡片 | 空卡模式、合并生成和评分排序共用 `.test-toggle` 标签卡片，整块点击切换原生复选框；沿用主页 12px 圆角和主题变量，点击时保持尺寸稳定，文字、背景和边框颜色使用 150ms 过渡；选中、键盘聚焦和禁用状态由内部输入控件决定，长标题在窄屏换行 |
