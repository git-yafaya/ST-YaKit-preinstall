...
| 试写色彩与文字 | 背景、卡片、表单和边框使用共享的 `--background`、`--paper`、`--field`、`--line`；创建按钮至少 40px 高，沿用主按钮的主题强调色及文字色；现代无衬线字体和 `-.01em` 字距，提示 12px / `--muted`，占位文字使用 `--yakit-text-muted`；Aa 空态保留衬线字体 |
...
| 文本输入框 | 通用样式聚焦时改变边框颜色；试写页输入框和下拉框聚焦增加由 `--focus` 派生的柔和 2px 外圈，文本框限制高度并关闭拖动缩放；按钮、复选框和单选项保留键盘焦点提示 |
